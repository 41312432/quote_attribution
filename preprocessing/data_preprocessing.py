...
def tokenize_and_locate_mention(raw_sentences_in_list, alias_to_id):
    """
    ...
    for sentence_index, sentence in enumerate(raw_sentences_in_list):
        #sentence tokenize
        tokenized_sentence = list(WordPunctTokenizer().tokenize(sentence))
...
def build_data_loader(data_file, alias_to_id, args, skip_only_one=False):
    ...
    for i, line in enumerate(progress_bar(data_lines, ncols=100, total=len(data_lines))):
        offset = i%26
        ...
        if offset == 22:
            ...
            tokenized_sentences, candidate_mention_positions = tokenize_and_locate_mention(raw_sentences_in_list, alias_to_id)
            ...
            one_hot_label = [0 if character_index != alias_to_id[speaker_name] else 1 for character_index in candidate_mention_positions.keys()]
